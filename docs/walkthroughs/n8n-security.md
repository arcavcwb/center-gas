...
- **[MODIFIED]** `workflows/n8n/WF-02_WhatsApp_Outbound.json`
  ...
  - Compara la cabecera `Authorization` entrante con `Bearer <SECURE_OUTBOUND_TOKEN>` (el token compartido entre Supabase y n8n; su valor real vive en la variable de entorno / Credential de n8n, nunca en el repositorio). Si no coincide, el flujo muere silenciosamente (Drop) sin gastar recursos ni disparar notificaciones fantasma.
